...
for(i in 1:nrow(election_2024)){
  squared_deviations[i] = (election_2024$R[i] - mean(election_2024$R))^2
}
```
...
for(i in 1:length(squared_deviations)){
  squared_deviation_sum = squared_deviation_sum + squared_deviations[i]
}
...
\textbf{Your turn}: Write a function that calculates the difference in `R` (from the elections data) for a given county between years 2024 and 2020 (\emph{hint: you only need one argument})

```{r eval=F}
get_diff <- function(county){
  election$R[election$Year == 2024 & election$County == county] -
    election$D[election$Year == 2024 & election$County == county]
}
```
...
```{r}
e_2020 <- sapply(counties, function(c) get_mean("E", c, 2020))
```

Do the same thing for 2024

```{r}
e_2024 <- sapply(counties, function(c) get_mean("E", c, 2024))
```

Now find the differences for each county from 2020 to 2024

```{r}
e_differences <- e_2024 - e_2020
```
...
```{r}
model <- lm(election_differences ~ e_differences, weights=weights)
```
...
ggplot(mapping=aes(x = e_differences, y = election_differences))+
  ...
  geom_line(aes(y = model$fitted.values), color="#88CCEE", linewidth=1.5)+
  theme_minimal()+
  labs(
...
From a practical standpoint, most counties voted for Trump regardless of the change in employment. And most counties experienced some kind of employment growth. The counties where the employment rate mattered the most were in \\large counties---and in those counties, the better the economy did over the past four years the more people, on average, voted more blue.
